...
function saveRankings(rankings) {
    fs_1.default.writeFileSync(rankingsFilePath, JSON.stringify(rankings, null, 2));
}
let rankings = loadRankings();
function gerarSlots(tipo) {
    const slots = {
        "2v2": ["**LIVRE**", "**LIVRE**"],
...
function isUserAuthorizedToRemove(interaction) {
    const member = interaction.member; // Aqui obtemos o membro da interação
...
exports.default = new command_1.Command({
    ...
    async run({ interaction, options }) {
        const tipoPartida = options.get("tipo")?.value;
        const slots = {
            "2v2": [1, 2, 3, 4],
            "3v3": [1, 2, 3, 4, 5, 6],
            "4v4": [1, 2, 3, 4, 5, 6, 7, 8],
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setColor('#00FFFF')
            .addFields({ name: "Time 1", value: "Nenhum Jogador", inline: true }, { name: "Time 2", value: "Nenhum Jogador", inline: true });
    ...
    buttons: new discord_js_1.Collection([
        ...
        ['removerJogador', async (interaction) => {
                const partida = partidas[interaction.message.id];
                if (!partida)
                    return interaction.reply({ content: "Erro ao encontrar a partida.", ephemeral: true });
                ...
                const selectMenu = new discord_js_1.StringSelectMenuBuilder()
                    ...
                    .addOptions(...[...partida.time1, ...partida.time2].map(playerId => new discord_js_1.StringSelectMenuOptionBuilder()
        ...
        ['definirVencedores', async (interaction) => {
                const partida = partidas[interaction.message.id];
                if (!partida)
                    return interaction.reply({ content: "Erro ao encontrar a partida.", ephemeral: true });
                if (!isUserAuthorizedToRemove(interaction)) {
                ...
                const selectMenu = new discord_js_1.StringSelectMenuBuilder()
                    ...
                    .setLabel("Time 1")
                    .setValue("time1"), new discord_js_1.StringSelectMenuOptionBuilder()
                    .setLabel("Time 2")
                    .setValue("time2"));
                const row = new discord_js_1.ActionRowBuilder({
                    components: [selectMenu],
    ...
    selects: new discord_js_1.Collection([
        ['vencedorSelect', async (interaction) => {
                await interaction.deferUpdate();
                const partida = partidas[interaction.message.id];
                console.log(partida);
                if (!partida) {
        ...
        ['mvpSelect', async (interaction) => {
                ...
                if (partida.vencedorDefinido) {
                    const resultadoEmbed = new discord_js_1.EmbedBuilder()
                        ...
                        .addFields({ name: "MVP", value: `${interaction.guild?.members.cache.get(mvpId)?.displayName}`, inline: true })
                        .setColor('#00FFFF')
        ...
        ['removerJogadorSelect', async (interaction) => {
                const partida = partidas[interaction.message.id];
                if (!partida) {
                ...
                const generateEmbed = () => {
                    const embed = new discord_js_1.EmbedBuilder()
                        ...
                        .setDescription('Escolha um time para jogar!')
                        .addFields({ name: 'Time 1', value: formatTime(partida.time1, partida.totalSlots), inline: true }, { name: 'Time 2', value: formatTime(partida.time2, partida.totalSlots), inline: true });
                    return embed;
                };
                const formatTime = (time, totalSlots) => {
                    if (time.length === 0) {
...
async function updateTeams(partida, interaction) {
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setDescription("Escolha um time para jogar!")
        .setColor('#00FFFF')
        .addFields({ name: "Time 1", value: slotsTime1.join("\n") || "Nenhum jogador", inline: true }, { name: "Time 2", value: slotsTime2.join("\n") || "Nenhum jogador", inline: true });
